  ...
  llmLoadPromise = (async () => {
    try {
        ...
        try {
          ...
          for (const cacheName of cacheNames) {
            if (
              ...
              console.log(`🗑️ Cleared corrupted cache: ${cacheName}`);
            }
          }
        } catch (cacheError) {
      ...
      offlineLLM = await pipeline(
        ...
        {
          // Force fresh download if corrupted
          cache_dir: "./.cache-voice-ai",
      ...
      console.log("🔄 Trying smaller backup model...");

      if (loadingProgressCallback) {
        loadingProgressCallback({
          text: "Trying smaller backup model...",
          percentage: 30,
        });
      }

      try {
        // Fallback to an even smaller, more reliable model
        offlineLLM = await pipeline(
          "text-generation",
          "Xenova/distilgpt2", // Much smaller and more reliable
          {
            cache_dir: "./.cache-voice-backup",
            progress_callback: (progress: any) => {
              if (loadingProgressCallback && progress) {
                const percentage = Math.min(
                  70,
                  30 + (progress.progress || 0) * 40
                );
                loadingProgressCallback({
                  text: `Loading backup model... (${Math.round(percentage)}%)`,
                  percentage,
                });
              }
            },
        ...
        console.log("✅ Offline LLM initialized with backup model!");
        return offlineLLM;
      } catch (cpuError) {
        console.error(
          "❌ Failed to initialize LLM even with backup:",
          cpuError
        );

        if (loadingProgressCallback) {
          loadingProgressCallback({
            text: "AI model failed - using keywords",
            percentage: 0,
          });
        }
...
export async function clearModelCache(): Promise<void> {
  ...
  try {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (
        key &&
        (key.includes("huggingface") || key.includes("transformers"))
      ) {
